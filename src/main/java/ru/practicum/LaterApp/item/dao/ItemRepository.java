...
import ru.practicum.LaterApp.item.model.ItemCountByUser;
import ru.practicum.LaterApp.item.model.ItemInfo;
...
public interface ItemRepository extends JpaRepository<Item, Long>, QuerydslPredicateExecutor<Item> {
    ...
    @Query("select new ru.practicum.LaterApp.item.model.ItemCountByUser(it.user.id, count (it.id)) " +
            "from Item as it " +
            "where it.url like ?1 " +
            "group by it.user.id " +
            "order by count (it.id) desc ")
    List<ItemCountByUser> countItemsByUser(String urlPath);
